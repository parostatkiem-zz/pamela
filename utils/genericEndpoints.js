...
export const createGenericListEndpoint = (kubeconfig, app) => (
  ...
  app.get(path, async (req, res) => {
    try {
      ...
      else res.status(500).send("Internal server error occured while listing resource");
    }
  });
};
...
export const createGenericGetEndpoint = (kubeconfig, app) => (
  ...
  app.get(path, async (req, res) => {
    try {
      ...
      console.error(e);
      if (e instanceof HttpError) e.send(res);
      else res.status(500).send("Internal server error occured while getting resource " + name);
    }
  });
};
